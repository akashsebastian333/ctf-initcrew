...
def profile_view(request, username):
    ...
    uaser = get_object_or_404(Profile, user__username=username)
    full_name = django_engine.from_string(uaser.full_name).render({}, request)
    addres = django_engine.from_string(uaser.address).render({}, request)
    phone = django_engine.from_string(uaser.phoneno).render({}, request)
    education = django_engine.from_string(uaser.skills).render({}, request)
    skills = django_engine.from_string(uaser.education).render({}, request)
    return render(request, 'profile_view.html', {'user_profile': uaser,'address': addres, 'full_name': full_name, 'phone': phone,'skills':skills,'education':education})


def profile_viewe(request, username):
    django_engine = engines['django']
    uaser = get_object_or_404(Profile, user__username=username)
    addres = django_engine.from_string(uaser.address)
    address=addres.render({}, request)
    print(address)
    return render(request, 'profile_view.html', {'user_profile': uaser,'address': address})


def profile_viewx(request, username):
    django_engine = engines['django']
    uaser = get_object_or_404(Profile, user__username=username)
    addres = f"{{{{{uaser.address}}}}}"
    print(addres)
    address = django_engine.from_string(addres).render({})
    return render(request, 'profile_view.html', {'user_profile': uaser, 'addrss': address})
...
def search_jobs(request):
    ...
    if job_posts.exists():
        ...
        error_message = django_engine.from_string('Sorry, we can\'t find this:' + query)
        print(error_message)
        h=error_message.render({}, request)
        #template = django_engine.get_template('search_jobs.html')
       # context = {'error_message': error_message}
        return render(request, 'search_jobs.html' ,{'error_message': h})
